...
from utils.CompareUtils import compare_to_number
...
class XAttributeDiscrete(XAttribute):
    ...
    def clone(self):
        ...
        """
        clone = XAttributeDiscrete(self.get_key(), self.__value,
                                   self.get_extension())
        return clone
    ...
    def compare_to(self, obj):
        ...
        if result == 0:
            return compare_to_number(self.__value, obj.get_value())
        return result
    ...
    def __eq__(self, other):
        if other is self:
            return True
        elif not isinstance(other, XAttributeDiscrete):
            return False
